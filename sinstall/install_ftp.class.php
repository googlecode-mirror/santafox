...
class install_ftp
{
    ...
    var $id_conect = false; //Линк на соединение с ФТП
    var $ftp_host = false;
    var $ftp_login = false;
    var $ftp_pass = false;
    ...
    function start()
    {
        ...
        switch ($step)
        {
            ...
            case 'step0':
                ...
                $need_exts = array("calendar", "session", "iconv", "json", "pcre", "mbstring", "gd", "mysql");
                ...
                case 'savestep2':
                    ...
                    $errore = '';
                    if ((empty($host)) || (empty($login)) /*|| (empty($pass))*/ || (empty($namebase)) || (empty($prefix)))
                            $errore = "Форма заполнена не полностью.";
                    else
                    {
    ...
    //========================================================================
               ...
               case 'step5':
                ...
                //Теперь надо уничожить созданные переменные, что бы мог нормально подключиться файл настроек
                   //Теперь пора запустить и сам непосредственный инсталл сделаем это прям тут
                   chdir('..');
    ...
    function mysql_install($etalon_install, $admin_login, $admin_pass)
    {
        ...
        include_once ("ini.php"); // Файл с настройками

        include_once ("include/kernel.class.php"); //Ядро
        include_once ("include/pub_interface.class.php");
        include_once ("include/mysql_table.class.php"); //Ядро
        include_once ("admin/manager_modules.class.php"); //Менеджер управления модулями
        include_once ("admin/manager_users.class.php"); //Менеджер управления модулями
    ...
    /**
     * Производит непосредственно процесс комирования движка на удаленный FTP
     *
     */
    function install()
    {
        ...
        print_r('<table width="100%" height="100%" border="0"><tr><td align="left" valign="top" id="content_iframe" style="font-size:9px"></td></tr></table>');
        print_r('<script>function show_message_content(show_msg)
                   {
        ...
        print_r('<script>parent.window.show_message("Подключение к удаленному серверу...");</script>');
        flush();
        ...
        print_r('<script>parent.window.show_message("Создание списка копируемых файлов...");</script>');
        flush();
        ...
        $array_copy = $this->get_array_file($this->id_conect, $this->link, $path, $this->cmod_for_limit);
        ...
        print_r('<script>parent.window.show_message("Создание файла настроек ini.php");</script>');
        flush();
        ...
        print_r('<script>parent.window.show_message("Создание файла инсталяции install.php");</script>');
        flush();
        ...
        print_r('<script>show_message_content("Нашмите &quot;Далее&quot; для продолжения интсталляции<br><br>")</script>');
        print_r('<script>show_message_content("Копирование файлов завершено<br><br>")</script>');
        print_r('<script>parent.window.show_message_hide();</script>');
        flush();
    }
    ...
    {
        //print_r('Реконект к FTP при создании: '.$dist.'<br>');
        //flush();
    ...
    function ftp_creat_dir($arr)
    {
        ...
        foreach ($arr as $dist => $mycmod)
        {
            ...
            if ($result === false)
            {
                //Произошла какая то ошибка при создании директоррии
                //пер
                if (!$this->ftp_creat_dir_bead($dist))
                {
                    print_r('<script>show_message_content("Директория так и не создана: '.$dist.'")</script>');
                    print_r('<script>show_message_content("Автоматическая инсталляция невозможна")</script>');
                    flush();
                    die();
                }
            }
            ...
            if ($result == 'NO')
                print_r('<script>show_message_content("Уже существует: '.$dist.'")</script>');
            else
            {
                $chmod_cmd="CHMOD ".$mycmod." ".$dist;
                ftp_site($this->id_conect, $chmod_cmd);
                print_r('<script>show_message_content("Создана директория: '.$dist.'")</script>');
            }
            }
            print_r('<script>parent.window.show_message_dir('.$i.','.count($arr).')</script>');
            flush();
            $i++;
        }
    }
    ...
    {
        print_r('<script>parent.window.show_message("Реконект к FTP при копировании: '.$dist.'");</script>');
        flush();
        //Прежде всего попробуем переконектится к ФТП
        $this->create_ftp_connect();
    ...
    function ftp_creat_file($arr, $mycmod)
    {
        ...
        foreach ($arr as $src => $dist)
        {
            ...
            if ($result === false)
            {
                ...
                {
                    print_r('<script>show_message_content("Не удалось скопировать файл: '.$dist.'")</script>');
                    print_r('<script>show_message_content("Автоматическая инсталляция невозможна")</script>');
                    flush();
                    die();
                }
            }
           //print_r('Пропущен '.trim($dst_dir."/".$file).'<br>');

                   //$chmod_cmd="CHMOD ".$mycmod[$dist]." ".$dist;
                   //ftp_site($conn_id, $chmod_cmd);
           }
               $this->curentsize += filesize($src);
               $size = round((($this->curentsize)/1024),0);
               print_r('<script>show_message_content("Скопирован '.$dist.'")</script>');
               print_r('<script>parent.window.show_message_files('.$size.','.$this->fullsize.')</script>');
               flush();
        }
    }
    ...
    function get_array_file($conn_id, $src_dir, $dst_dir, $mycmod)
    {
        ...
        while($file = $d->read())
        {
            ...
            else
            {
                ...
                //$size_dist = ftp_size($conn_id, $dst_dir."/".$file);
                //if ($size_dist != $size_src)
                //{
                //}
    ...
    function get_all_modules()
    {
        ...
        $d->close();
        //$this->debug($ret_array);
        return $ret_array;
    }


    function redirect_refresh($url)
    {
